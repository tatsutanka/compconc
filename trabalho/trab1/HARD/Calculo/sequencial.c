...
/*O objetivo do trabalho é o calculo do determinante de uma matriz atravez do
...
double  det_laplace(int size_matriz , double* matriz){
    ...
    for( i = 0; i < size_matriz;i++){
        if(matriz[0*size_matriz+i] != 0){
            ...
            printf(";;Factor:%f\n",factor);
            printf("**Det:%f\n",det);
            for (int m =0;m<size_matriz;m++){
                for(int n = 0 ;n<size_matriz;n++){
                    printf("%f ",aux[m*size_matriz+n]);
                }
                printf("\n");
            }
            printf("--size:%d\n",size_matriz);
