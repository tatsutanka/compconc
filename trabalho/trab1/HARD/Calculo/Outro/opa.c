...
long int determinant(int n,long int *arr) 
{ 
    long int i,j,k,sum=0,arr1[n-1][n-1]; 
    if(n>1){ 
...
int main(void){
    int size = 3;
    long int *arr = (long int*) malloc(sizeof(long int)*size*size);
    ...
    for(int i = 0; i < size;i++){
        ...
        for(int j =0 ;j<size;j++){
            printf("%ld ",arr[i*size+j]);
        }
        printf("\n");
    }
    printf("\n");
    printf("determinante:%ld\n",determinant(size,arr));
    return 0;
}
